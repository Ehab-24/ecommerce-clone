...
export default function GiftCardsDatable({ initialGiftCards }: { initialGiftCards: GiftCard[] }) {
...
function getHeaderItems(products: GiftCard[]): HeaderItem<GiftCard>[] {
  return [
    {
      label: "Code ending", sortable: true, onSort: (sortKey) => {
...

    {
      label: "Supplier", sortable: true, onSort: (sortKey) => {
        let sortedGiftCards = [...products]
        switch (sortKey) {
          case "desc": sortedGiftCards.sort((a, b) => a.customer.firstName.localeCompare(b.customer.firstName)); break;
          case "asc": sortedGiftCards.sort((a, b) => b.customer.firstName.localeCompare(a.customer.firstName)); break;
          default: throw new Error("Sort type not allowed")
        }
        return sortedGiftCards
      }
    ...
    {
      label: "Recipient", sortable: true, onSort: (sortKey) => {
        let sortedGiftCards = [...products]
        switch (sortKey) {
    ...
    {
      label: "Date issued", sortable: true, onSort: (sortKey) => {
        let sortedGiftCards = [...products]
        switch (sortKey) {
          case "desc": sortedGiftCards.sort((a, b) => {
            if (a.createdAt && b.createdAt) {
              return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
            }
            return 0
          }); break;
          case "asc": sortedGiftCards.sort((a, b) => {
            if (a.createdAt && b.createdAt) {
...
function getAllFilters(): FilterElements {
  ...
  return {
    ...
    "Gift card value": <div className="flex flex-col gap-1">Gift card value</div>,
