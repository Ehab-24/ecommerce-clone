...
export default function Datable({ initialCollections }: { initialCollections: Collection[] }) {
    ...
    return (
      <tr className="bg-white border-b hover:bg-gray-50 ">
        <td className="w-4 p-4">
          <Checkbox id={"select-" + c._id} checked={isSelected} onChange={e => setSelected(e.target.checked)} />
        </td>
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="rounded-xl p-1.5 bg-white flex flex-col">
        ...
        <Button variant="ghost" className="py-1 px-2 flex items-start text-xs text-gray-800 rounded-lg">
          Delete collections
        </Button>
...
function getHeaderItems(products: Collection[]): HeaderItem<Collection>[] {
  return [
    {
      label: "Title", sortable: true, onSort: (sortKey) => {
        let sortedCollections = [...products]
        switch (sortKey) {
          case "desc": sortedCollections.sort((a, b) => a.title.localeCompare(b.title)); break;
          case "asc": sortedCollections.sort((a, b) => b.title.localeCompare(a.title)); break;
