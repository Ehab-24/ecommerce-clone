    ...
    <Dialog open={open} onOpenChange={o => {
      ...
      <DialogContent className="h-min">
          ...
          <div className="flex items-center justify-between">
            {
              selectedLocations.length === locations.length ? (
                <TextButton onClick={() => setSelectedLocations([])}>Deselect all</TextButton>
              ) : (
                <TextButton onClick={() => setSelectedLocations([...locations])}>Select all</TextButton>
              )
            }
            <Text className="text-sm text-gray-800">{selectedLocations.length} of {locations.length} selected</Text>
          </div>

          <div className="flex flex-col gap-2">
            {
              locations.map(l => (
                <Checkbox key={l._id} id={`select-lcoation-${l._id}`} label={l.name} checked={!!selectedLocations.find(_l => _l._id === l._id)} onChange={(e) => {
                  if (e.target.checked) {
                    setSelectedLocations([...selectedLocations, l])
                  } else {
                    setSelectedLocations(selectedLocations.filter(l => l._id !== l._id))
                  }
                  console.log(l._id, selectedLocations)
                }} />
              ))
            }
        ...
        <DialogFooter className="flex gap-2">
          ...
          <FilledButton onClick={() => { onSave(selectedLocations); setOpen(false); }}>Done</FilledButton>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
