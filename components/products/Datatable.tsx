  ...
  function Row({ item: p, isSelected, setSelected }: RowProps<Product>) {
    return (
      ...
      >
        ...
        >
          {p.media?.length > 0 &&
            p.media.map((m, i) => (
              <div
                key={i}
                className=" aspect-square h-8 bg-gray-200 rounded-md overflow-hidden"
              >
                <Image
                  width="32"
                  height="32"
        ...
        <td className="px-6 py-2">{p.quantity} in stock</td>
        <td className="px-6 py-2">-</td>
        <td className="px-6 py-2">-</td>
        <td className="px-6 py-2">{p.category}</td>
        <td className="px-6 py-2">{p.vendor?.name}</td>
      </tr>
    );
  }
...

    {
      ...
      onSort: (sortKey) => {
        ...
        switch (sortKey) {
          case "desc":
            sortedProducts.sort(
              (a, b) => a.vendor?.name?.localeCompare(b.vendor?.name ?? "") ?? 0
            );
            break;
          case "asc":
            sortedProducts.sort(
              (a, b) => b.vendor?.name?.localeCompare(a.vendor?.name ?? "") ?? 0
            );
            break;
          default:
            throw new Error("Sort type not allowed");
