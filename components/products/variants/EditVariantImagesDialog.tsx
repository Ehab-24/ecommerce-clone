...
export default function EditVariantImagesDialog({ onSave, image, altText, initialImages, text = "Update variant image", button }: { button?: React.ReactNode, image?: string | null, initialImages: string[], text?: string, altText: string, onSave: (selectedImage: string | null, newImages: string[]) => void }) {

  const [images, setImages] = React.useState<string[]>([])
  const [open, setOpen] = React.useState(false);
  const [selectedImage, setSelectedImage] = React.useState<string | null>(image ?? null)

  return (
    <Dialog open={open} onOpenChange={v => { setImages([]); setOpen(v) }}>
      <DialogTrigger asChild>
        {
          button ? button : (
            <button onClick={() => setOpen(true)} className={`rounded-md overflow-hidden grid self-center mr-2 hover:bg-gray-100 transition-all hover:border-gray-500 hover:text-gray-800 text-gray-300 place-items-center bg-white h-14 w-14 min-w-[56px] ${image ? "" : "border-2 border-gray-300 border-dashed "}`} >
              {
                image ? (
                  <Image src={image} alt={altText} width={0} height={0} sizes="100vw" style={{ width: '100%', height: '100%' }} />
                ) : (
              ...
              initialImages.map((url) => {
                return (
                  <div key={url} onClick={() => setSelectedImage(url)} className={`h-40 w-40 cursor-pointer bg-gray-100 border-2 rounded-xl overflow-hidden relative ${selectedImage === url ? "border-blue-700" : "hover:border-blue-700 border-white"}`}>
                    ...
                    {
                      selectedImage === url && <IoIosCheckmarkCircleOutline size={20} className="bg-white rounded-full text-blue-700 absolute top-2 left-2" />
                    }
                  </div>
                )
              })
