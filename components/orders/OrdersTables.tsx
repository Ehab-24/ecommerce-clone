...
export default function Datatable({ orders }: { orders: Order[] }) {
  ...
  const headerItems: HeaderItem[] = [
    ...
    { label: "Fulfilment status", sortable: true, sortKey: "fulfilment_status" },
  ...
  const [selectedOrders, setSelectedOrders] = useState<boolean[]>(new Array(orders.length).fill(false))
        ...
        <thead className="text-[10px] text-gray-700 uppercase bg-gray-100 border-t-2 border-b-2 ">
          <tr>
            <th scope="col" className="p-4">
              <Checkbox id="select-all-products" label={selectedOrders.some(p => p) ? selectedOrders.filter(p => p).length + " selected" : ""} checked={allChecked} onChange={e => setSelectedOrders(new Array(orders.length).fill(e.target.checked))} />
            </th>
            {
              headerItems.map(h => (
                <th key={h.label} scope="col" className="px-6 py-1">
                  {selectedOrders.some(p => p) ? "" : h.sortable ? <SortableHeader header={h} onClick={() => { }} sorted="none" /> : h.label}
                </th>
              ))
            }
          </tr>
            ...
            orders.map((p, i) => (
              <tr key={p._id} className="bg-white border-b hover:bg-gray-50 ">
                <td className="w-4 p-4">
                  <Checkbox id={"select-" + p._id} checked={selectedOrders[i]} label="" onChange={e => {
