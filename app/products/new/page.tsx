...
import { ApiProductSchema, Variant, ApiProduct, VariantOption } from "@/types/product";
...
export default function NewProductPage() {
  ...
  useEffect(() => {
    setProduct(p => ({ ...p, variants: multiplyArrays(...p.variantOptions.map(v => v.values)).map(arr => ({ name: arr.join(" / "), values: arr, price: p.price, costPerItem: p.costPerItem, profit: p.profit, margin: p.margin })) }))
  ...
  function multiplyArrays(...arrays: string[][]): string[][] {
...

      {
        product.variants.length > 0 && (
          <div className="flex mt-4 justify-between border-t border-gray-200 p-4">
            ...
            {
              selectedVariants.length === 0 && <OutlinedButton onClick={() => { }}>
