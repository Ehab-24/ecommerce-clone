  ...
  return (
    <div className="flex w-full justify-between">
      ...
      <div className="flex w-max items-center">
        <OutlinedButton>Duplicate</OutlinedButton>
        ...
        {vi !== undefined && (
          <div className=" hidden  md:flex">
            <Link
              href={`/products/${product!._id}/variants/${vi - 1}`}
              aria-disabled={vi === 0}
              className={`${
                vi === 0
                  ? "pointer-events-none text-gray-400"
                  : "hover:bg-gray-300 "
              } border border-gray-300 bg-gray-200 py-1 px-2 grid place-items-center rounded-tl-md rounded-bl-md transition-all`}
            >
              <IoIosArrowBack size={14} />
            </Link>
            <Link
              href={`/products/${product!._id}/variants/${vi + 1}`}
              aria-disabled={product!.variants.length === vi + 1}
              className={`${
                product!.variants.length === vi + 1
                  ? "pointer-events-none text-gray-400"
                  : "hover:bg-gray-300 "
              } border-t border-b border-r border-gray-300 bg-gray-200 px-2 py-1 grid place-items-center rounded-tr-md rounded-br-md transition-all`}
            >
    ...
    <div className="md:max-w-[280px] w-full flex flex-col gap-4">
        ...
        <div className="flex w-full gap-4">
          {product.media.length > 0 ? (
            ...
            <Link
              ...
              className="w-16 h-16 rounded-md overflow-hidden border border-gray-300 grid place-items-center"
            >
      ...
      <Card className="pt-4 overflow-hidden">
        ...
        {product.variants.map((v, i) => (
          <Link
            key={i}
            href={`/products/${product._id}/variants/${i}`}
            className="hover:bg-gray-100/60 bg-white flex items-center gap-2 px-4 transition-all border-t border-gray-200 py-2"
          >
            {v.image ? (
              <p>hello</p>
            ) : (
